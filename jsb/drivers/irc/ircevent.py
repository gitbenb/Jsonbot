...
from jsb.utils.generic import toenc, fromenc, strippedtxt, fix_format, stripident
...
class IrcEvent(EventBase):
    ...
    def parse(self, bot, rawstr):
        ...
        try: 
            ...
            self.nick = stripident(nickuser[0])
    ...
    def reply(self, txt, result=[], event=None, origin="", dot=u", ", nr=375, extend=0, *args, **kwargs):
        ...
        if self.isdcc: self.bot.say(self.sock, txt, result, how, self, nr, extend, dot, *args, **kwargs)
        elif self.msg: self.bot.say(self.nick, txt, result, how, self, nr, extend, dot, *args, **kwargs)
        elif self.silent or (self.chan and self.chan.data and self.chan.data.silent): self.bot.say(self.nick, txt, result, how, self, nr, extend, dot, *args, **kwargs)
        else: self.bot.say(self.channel, txt, result, how, self, nr, extend, dot, *args, **kwargs)
        return self
