...
""" Defines an on-demand importer that only actually loads modules when their
    ...
    6-7-2012 BHJTW adapted for JSONBOT 
...
class OnDemandLoader(object):
    ...
    def load_module(self, fullname):
        logging.info("importing %s from %s" % (fullname, whichmodule(1)))
        if 'jsb.plugs' in fullname or 'myplugs' in fullname: return _import(fullname)
        if fullname == "sys": return None
...
class OnDemandImporter(object):
    ...
    def find_module(self, fullname, path=None):
        if 'jsb.plugs' in fullname or 'myplugs' in fullname: return None
        if fullname == "sys": return None
        origName = fullname
        if not path:
