...
class Users(Persist):
    ...
    def byname(self, name):
        ...
        try:
            ...
            if user.data.userhosts:
                if user.data.deleted: logging.error("%s user is deleted" % name) ; return
                else: return user
    ...
    def merge(self, name, userhost):
        ...
        if user:
            if not userhost in user.data.userhosts:
                ...
                user.save()
            self.data.names[userhost.lower()] = name
            self.save()
    ...
    def getuser(self, userhost):
        ...
        try:
            if userhost: userhost = userhost.lower()
            cacheentry = self.data.names[userhost]
            if cacheentry: logging.info("cache entry is %s" % cacheentry)
            user = self.byname(cacheentry)
            if user: return user
            else: logging.info("can't find %s user" % cacheentry)
        except KeyError:
            logging.info("can't find %s in names cache" % userhost) 
    ...
    def add(self, name, userhosts, perms):
        ...
        for userhost in userhosts: self.data.names[userhost.lower()] = name
        newuser.save()
        self.save()
        logging.warn('%s (%s) added to user database - %s' % (name, userhosts, ", ".join(perms)))
        return newuser
        ...
        try:
            ...
            if user:
                 for userhost in user.data.userhosts:
                     try: del self.data.names[userhost.lower()]
                     except KeyError: logging.warn("can't delete %s from names cache" % userhost.lower())
            self.save()
            return True
        except NoSuchUser:
            pass
    ...
    def deluserhost(self, name, userhost):
        ...
        if user:
            if userhost in user.data.userhosts:
                ...
                user.save()
            try: del self.data.names[userhost.lower()] ; self.save()
