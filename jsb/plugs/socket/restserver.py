...
def startserver(force=False):
    ...
    try:
        server = RestServer((int(cfg.get('host')), int(cfg.get('port'))), RestRequestHandler)
        if server:
            server.start()
            logging.warn('running at %s:%s' % (cfg.get('host'), cfg.get('port')))
            for mount in cfg.get('disable'): server.disable(mount)
        else: logging.error('failed to start server at %s:%s' % (cfg.get('host'), cfg.get('port')))
    except ValueError, ex: logging.warn("config is not correct: %s" % str(ex))
    except socket.error, ex: logging.warn('start - socket error: %s' % str(ex))
...
def stopserver():
    """ stop server. """
    try:
        if not server: logging.debug('server is already stopped') ; return
        server.shutdown()
    except Exception, ex: handle_exception()
