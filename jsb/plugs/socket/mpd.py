...
cmnds.add('mpd-whatsnext',    handle_mpd_whatsnext,    'USER', threaded=True)

def handle_mpd_filelink(bot, ievent):
    try:
        status = MPDDict(mpd('currentsong'))
        status['time'] = mpd_duration(status['time'])
        filename = mpd_duration(status['file'])
        ievent.reply(cfg.get('filelink') % urllib.quote('%s' % status['file']))
    except MPDError, e:
        ievent.reply(str(e))

cmnds.add('filelink',  handle_mpd_filelink,  'USER')
