...
def forwardoutcb(bot, event): 
    ...
    for jid in forward.data.channels[event.channel.lower()]:
        if not "@" in jid and jid != "twitter": logging.error("forward - %s is not a valid JID" % jid) ; continue
        logging.info("forward - sending to %s" % jid)
        if jid == "twitter":
            if event.cbtype == "JOIN": continue
            txt = bot.normalize(txt)
            try: twitter_out(forward.data.outs[jid], txt, event)
            except KeyError: logging.warn("no forwards found for %s" % jid)
