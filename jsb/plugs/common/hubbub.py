...
from jsb.api.hooks import get_hooks, post_hooks
...
def subscribe(url, host):
    ...
    subscribe_args = {
        'hub.callback': urlparse.urljoin('https://', host, '/hubbub'),
        'hub.mode': 'subscribe',
        'hub.topic': url,
        'hub.verify': 'async',
...
class HubbubItem(Persist):
    ...
    def __init__(self, name, url="", owner="", itemslist=['title', 'link'], watchchannels=[], running=1):
        filebase = getdatadir() + os.sep + 'plugs' + os.sep + 'jsb.plugs.common.hubbub' + os.sep + name
...
def handle_hubbubadd(bot, ievent):
    ...
    result = subscribe(url, socket.gethostname())
