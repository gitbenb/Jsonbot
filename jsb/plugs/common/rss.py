...
from jsb.contrib.natural.date import duration, delta
...
sleeptime=15*60, running=0):
    ...
    def deliver(self, datalist, save=True):
        ...
        try:
            ...
            for item in loopover:
                ...
                if self.markup.get(jsonstring([name, type, channel]), 'all-lines'):
                    for i in res2: 
                        ...
                        try: bot.say(nick or channel, response, how=how, speed=-1, plugorigin="rss", **{'headlines': True}) ; nritems += 1
            ...
            return nritems
        except Exception, ex: handle_exception(txt=name) ; return str(ex)
    ...
    def makeresponse(self, name, type, res, channel, sep=" || "):
        ...
        for j in res:
            ...
            for i in itemslist:
                ...
                except (UnicodeDecodeError, KeyError, AttributeError, TypeError), ex: logging.warn('%s - %s' % (name, str(ex))) ; continue
        ...
        if feed:
            ...
            if result:
                ...
                if res2:
                    ...
                    sl = sleeptime.data[name]
                    if sl > 300: sleeptime.data[name] = 120
                else:
                    logging.info("%s - nothing to deliver" % name)
                    sl = sleeptime.data[name] 
                    if sl < 1800: sleeptime.data[name] += (sl*0.1 + random.randint(1,10))
...
def dosync(feedname):
    ...
    except RssException, ex: logging.error("%s - error: %s" % (feedname, str(ex))) ; return str(ex)
...
def handle_rsslastpoll(bot, event):
    if not event.rest:
        result = {}
        for feedname, last in lastpoll.data.iteritems():
            if last - starttime < 0: last = starttime
            try: result[feedname] = duration(last, plain=True)
            except Exception, ex: logging.warn("error determining lastpoll of %s: %s" % (feedname, str(ex))) ; continue
    ...
    except Exception, ex: event.reply("error occured: %s" % str(ex))

cmnds.add("rss-lastpoll", handle_rsslastpoll, ["OPER", "USER"])
examples.add("rss-lastpoll", "shows time when a feed was last polled", "1) rss-lastpoll jsonbot 2) rss-lastpoll all")
...
def handle_rssnextpoll(bot, event):
    if not event.rest:
        result = {}
        for name, last in lastpoll.data.iteritems():
            sleep = sleeptime.data[name]
            if last - starttime < 0: last = starttime
            try: result[name] = duration(sleep + last, precision=0.1, plain=True)
            except Exception, ex: logging.warn("error determining lastpoll of %s: %s" % (name, str(ex))) ; continue
        event.reply("lastpolled: ", result)
        return
    name = event.args[0]
    try:
...
examples.add("rss-nextpoll", "shows time until a feed will be polled", "1) rss-nextpoll jsonbot 2) rss-nextpoll all")
...
def handle_rssadditem(bot, ievent):
    ...
    for name in todo:
        ...
        except Exception, ex: errors[name] = str(ex) ; continue
        done.append(name)
    ...
    for name in todo:
        try:
            ...
            try:
                ...
                rssitem.itemslists.save()
            except (ValueError, KeyError), ex: errors[name] = str(ex) ; continue
        except Exception, ex: errors[name] = str(ex) ; continue
        done.append(name)
