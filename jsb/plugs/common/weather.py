...
def handle_weather(bot, ievent):
    ...
    resultstr = ""
    if weathertxt:
        try: gweather = minidom.parseString(weathertxt)
        ...
        if ievent.usercmnd == "weather":
            ...
            if info:
                ...
                resultstr = "%s (%s) has a temperature of %sC/%sF with %s. %s (%s km/h).%s" % (city, zip, temp_c, temp_f, humidity, wind, wind_km, condition)
