...
def handle_unignore(bot, event):
    ...
    userhost = getwho(bot, nick)
    if not userhost: userhost = event.rest
    if userhost in bot.state['ignore']: bot.state['ignore'].remove(userhost) ; event.reply("%s removed from ignore list" % userhost)
...
def handle_ignorelist(bot, event):
    event.reply("%s ignore list: " % bot.cfg.name, bot.state['ignore'])

cmnds.add("ignore-list", handle_ignorelist, ["OPER", ])
examples.add("ignore-list", "show the ignore list of a bot", "ignore-list")
