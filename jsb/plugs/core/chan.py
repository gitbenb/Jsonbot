...
from jsb.lib.persist import Persist, findnames
...
def handle_chanjoin(bot, ievent):
    """ arguments: <channel> - join a channel/conference"""
...
## chan-listdisk command

def handle_chanlistdisk(bot, ievent):
    """ no arguments - channels .. show joined channels of diskdrive. """
    path = getdatadir() + os.sep + 'fleet' + os.sep + stripname(bot.cfg.name) + os.sep + 'channels'
    chans = findnames(path)
    if chans: ievent.reply("channels on disk: ", chans)
    else: ievent.reply('no channels found')

cmnds.add('chan-listdisk', handle_chanlistdisk, ['USER', 'GUEST'])
examples.add('chan-listdisk', 'show what channels are on disk (for the bot command is given on)', 'chan-listdisk')
...
def handle_chanwithnotice(bot, ievent):
    ...
    if ievent.rest: channel = ievent.rest.split()[0].lower()
        ...
        channel = ievent.channel
    if not ievent.msg:
        if not ievent.user or not "OPER" in ievent.user.data.perms: ievent.reply("you need to be an OPER to change this channel 'how' parameter") ; return
    ievent.reply('setting notice in %s' % channel)
    ievent.chan.data.how  = "notice"
    ievent.chan.save()
    ievent.done()
    
cmnds.add('chan-withnotice', handle_chanwithnotice, ['OPER', 'USER'])
...
cmnds.add('chan-withprivmsg', handle_chanwithprivmsg, ['OPER', 'USER'])
...
def handle_chandeltag(bot, event):
    """ arguments: <plugname> - delete a tag from the channels taglist. """
    if not event.rest: event.missing("<tag>") ; return
    if event.rest in event.chan.data.taglist:
        event.chan.data.taglist.remove(event.rest)
        event.chan.save()
        event.done()
    else: event.reply("%s is not in the taglist of channel %s" % (event.rest, event.channel))

cmnds.add("chan-deltag", handle_chandeltag, 'OPER')
examples.add("chan-deltag", "delete a tag from the channels taglist", "chan-deltag #jsonbot")
