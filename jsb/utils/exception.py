...
def handle_exception(event=None, log=True, txt="", stop=False, target=None):
    """ handle exception.. for now only print it. """
    errormsg = exceptionmsg()
    if txt: errormsg = u"%s - %s" % (txt, errormsg)
    if log: logging.error(RED + txt + u" " + errormsg + ENDC)
    if event:
        ...
        if event.bot:
            ...
            if target: event.bot.saynocb(target, u"*sorry* - an exception occured - %s" % errormsg)
